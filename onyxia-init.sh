...
# Log to file while still streaming to stdout/stderr.
LOG_FILE="/opt/onyxia-init.log"
...
# Determine if sudo is allowed; if yes, refresh CA certs and use system-wide env persistence.
sudo true -nv 2>&1
if [ $? -eq 0 ]; then
  echo "sudo_allowed"
  export SUDO=0
...
# Optional region-specific init hook (temporary insecure fetch for on-prem instances).
...
# Optional Vault integration: fetch secrets and export them as env vars.
if  [[ -n "$VAULT_RELATIVE_PATH" ]]; then
    ...
    # obtained above), read secrets and export them as env vars.
    if  [[ -n "$VAULT_TOKEN" ]]; then
        ...
        # Persist env vars system-wide if possible; otherwise to user shell/R env.
        for i in $KEYS;
        do
            echo $i
...
# If kubectl is available, configure in-cluster context and expose SA info.
...
# R/RStudio integration: append env vars and project settings.
if command -v R &>/dev/null; then
...
# Optional dark mode configuration for JupyterLab, code-server, and RStudio.
...
# Inject proxy variables into system environment (and R if present) when sudo is allowed.
...
# Configure duckdb CLI and S3 secret if credentials are present.
...
# Delegate repository configuration (R/CRAN, pip) to the shared script.
source /opt/onyxia-set-repositories.sh

# Optional personal init hook for user-specific setup.
if [[ -n "$PERSONAL_INIT_SCRIPT" ]]; then
    echo "download $PERSONAL_INIT_SCRIPT"
    curl $PERSONAL_INIT_SCRIPT | bash -s -- $PERSONAL_INIT_ARGS
fi

# Fix ownership of project directory contents for the target user/group.
